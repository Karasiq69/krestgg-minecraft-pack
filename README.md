...
- **Производительность** — Sodium.
- **Iris (шейдеры)** — **по желанию, по умолчанию выключен.** На Minecraft 26.2 у Iris есть
  баг: у части предметов в инвентаре и рецептах пропадают иконки
  ([Iris #3212](https://github.com/IrisShaders/Iris/issues/3212)). Хотите шейдеры — включайте
  галочку при установке и чините пропавшие иконки нажатием `F3+T`. Когда апстрим починит —
  вернём Iris в обязательные.
- **Distant Horizons** — дальняя прорисовка, **по желанию**. При установке пака появится галочка,
  по умолчанию выключена: мод красивый, но тяжёлый (просаживает FPS и весит 30 МБ). Передумали —
  в Prism кнопка `Edit` → `Optional mods`.
...
> ⚠️ **Не обновляйте Sodium вручную.** Iris цепляется к Sodium миксинами и работает только с той
> версией, которую сам требует (сейчас `0.9.1`). Более свежая альфа Sodium ломает загрузку мира:
> игра падает при входе на сервер. Версия в паке подобрана под Iris — просто не трогайте её.

> ⚠️ **Ставили KrestMC.zip до версии 3.0.1?** У старого инстанса Distant Horizons ставился всем
> без вопроса. Проще всего удалить инстанс и импортировать свежий `KrestMC.zip` из релиза (мир
> хранится на сервере, ничего не потеряется). Либо вручную: в Prism `Edit` → `Settings` →
> `Custom commands` → убрать флаг `-g` из pre-launch команды, удалить `.minecraft/packwiz.json` и
> jar Distant Horizons из `mods/` — при следующем запуске инсталлер спросит, какие необязательные
> моды ставить.
>
> В Modrinth App (`KrestMC.mrpack`) лаунчер может поставить Distant Horizons сам — на слабом ПК
> просто удалите его jar из `mods/` или выключите мод в лаунчере.
